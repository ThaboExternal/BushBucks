...
`.github/workflows/azure-static-web-apps.yml`, already pointed at the right
folders:
- **App location**: `app`
- **API location**: `api`
- **Output location**: *(blank — plain HTML, no build step)*

To use it:

1. Push this repo to GitHub.
2. In the Azure Portal, create the Static Web App and link it to this repo/branch
   (or run `az staticwebapp create ...` against the repo). Azure will generate
   a deployment token and store it as a GitHub secret automatically.
3. This workflow is already wired to the
   `AZURE_STATIC_WEB_APPS_API_TOKEN_ICY_HILL_0D3FC9310` secret (the one Azure
   generated for this app). If you ever recreate the Static Web App resource,
   Azure will generate a new token under a new secret name — update the two
   `azure_static_web_apps_api_token:` lines in `azure-static-web-apps.yml` to
   match. Also delete Azure's auto-generated workflow file if it ends up as a
   second, competing one in `.github/workflows/`.

Or deploy directly with the SWA CLI (no GitHub Actions needed):
